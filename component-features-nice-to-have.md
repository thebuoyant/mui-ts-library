...
| Spalten-Management-UI | `enableColumnManagement?: boolean` (Default `false`) — Spalten-Header zeigen ✏️/🗑️-Icons (immer sichtbar), am Board-Ende ein „Spalte hinzufügen"-Button. Umbenennen inline im Header (Enter/Blur/Escape). Löschen öffnet Bestätigungsdialog mit Karten-Anzahl-Warnung; Löschen entfernt auch alle Karten der Spalte und feuert `onTasksChange`. 4 neue Callbacks: `onColumnsChange`, `onColumnAdd`, `onColumnUpdate`, `onColumnDelete`. 6 neue Translation-Keys: `columnAddLabel`, `columnAddPlaceholder`, `columnDeleteConfirm`, `columnDeleteCardsWarning`, `columnRenameTooltip`, `columnDeleteTooltip`. 2 neue CSS-Klassen: `MuiTsKanbanBoard-columnActions`, `MuiTsKanbanBoard-columnAddButton`. | Mittel | — |
