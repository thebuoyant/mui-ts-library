...
| Karten-Subtasks | `subtasks?: KanbanSubtask[]` auf `KanbanTask` (`KanbanSubtask = { id, title, done }`) — auf der Karte als Fortschrittsbalken (`2 / 5 ✓`) mit kleinem `+`-Hover-Button (Tooltip, öffnet Edit-Dialog via `stopPropagation`), im Add/Edit-Dialog als Checklist (Toggle/Add per Enter/`+`-Button, Remove per `×`). `showSubtasks?: boolean` (Default `true`) blendet Bar & Checklist aus. 3 neue Translation-Keys: `dialogFieldSubtasks`, `dialogSubtaskAdd`, `cardSubtaskAdd`. 2 neue CSS-Klassen: `MuiTsKanbanBoard-cardSubtasks`, `MuiTsKanbanBoard-cardSubtasksBar`. | Mittel | — |
