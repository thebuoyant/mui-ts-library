...
- **KanbanBoard — `translation.dialogFieldSubtasks` / `translation.dialogSubtaskAdd`**: New translation keys for the subtask checklist section label and the "Add subtask" input placeholder.
...
- **README changelog sections (EN + DE):** The embedded changelog in `README.md` and `README.de.md` (what npmjs.com displays) was missing the `[3.25.0]` and `[3.26.0]` entries — they were added after the publish. No code changes.
...
- **`RichTextEditor` toolbar:** `H1`/`H2`/`H3` and `MD` buttons are now rendered as `<svg><text>` instead of an HTML `<span>`. This makes them visually consistent with the surrounding Material SVG icons — same rendering engine, same `fill="currentColor"` color behaviour, and `fontWeight="500"` matching the stroke weight of outlined icons. Previously the bold HTML text looked heavier and slightly misaligned compared to the SVG icons.
- **`RadialStackedBarChart` stories:** removed unused `_key` parameter from the `valueFormatter` story example that caused an ESLint `no-unused-vars` error.
...
#### CSS Classes API (Slot Classes) — `TagSelection`, `PasswordStrengthMeter`, `ColorPicker`

Every significant DOM node in all three components now carries a stable, documented CSS class name. Consumers can style individual slots via plain CSS, CSS Modules, Tailwind, or any other CSS approach without relying on MUI's internal class names.

**Convention**
- Slot classes: `.MuiTs<Component>-<slot>` (e.g. `.MuiTsTagSelection-chip`)
- Shared state classes: `.MuiTs-disabled`, `.MuiTs-selected`, `.MuiTs-focused`, `.MuiTs-error`
- Strength-state classes (PasswordStrengthMeter): `.MuiTsPasswordStrengthMeter-strengthBarWeak/Ok/Good/VeryGood`

**New exports**

Three typed constants objects are now exported from the library — import them to reference class names safely without magic strings:
...
**Fully backwards-compatible** — no existing props or behaviour changed. All classes are purely additive.
...
#### D3 Charts — `valueFormatter` prop (4 charts)

New prop `valueFormatter` for all D3 charts that display numeric values in tooltips. Overrides the existing `valueDecimalCount` / `valueDecimalSeparator` / `valueThousandsSeparator` props when provided. All four changes are purely additive — no existing behavior changes.

- **`RadialStackedBarChart`**: `valueFormatter?: (value: number, seriesKey: string) => string` — use `seriesKey` to apply different units per series (e.g. `€` for revenue, `kg` for weight).
- **`ChordChart`**: `valueFormatter?: (value: number) => string` — formats all numeric values in group and chord tooltips.
- **`SunburstChart`**: `valueFormatter?: (value: number) => string` — formats the aggregate value shown in segment tooltips.
- **`CirclePackingChart`**: `valueFormatter?: (value: number) => string` — formats node values in tooltips (previously always `toLocaleString()`).
...
- New prop `chipVariant?: 'filled' | 'outlined'` (default: `'filled'`) controls the MUI Chip variant for all tag chips — both in the selected-tags area and in the autocomplete dropdown options.
- Consumers using a design system that prefers `outlined` chips can now set `chipVariant="outlined"` instead of overriding styles manually.
...
New prop `onMentionInserted?: (item: MentionItem) => void` — fires immediately after the user selects an item from the `@` mention dropdown. The callback receives the full `MentionItem` (`{ id, label }`), so the consumer knows exactly who was mentioned without parsing the HTML output.

```tsx
<RichTextEditor
  mentionItems={users}
  onMentionInserted={(item) => {
    console.log(`${item.label} (${item.id}) was mentioned`);
    notifyUser(item.id);
  }}
/>
```

Only active when `mentionItems` or `onMentionSearch` is provided (i.e. when the Mention extension is enabled). Purely additive — no existing behaviour changes.
...
#### RichTextEditor — Paste as Plain Text

- `showPasteAsPlainTextButton` (default `false`): adds a toolbar toggle. When active, all pasted content — rich HTML from a website, a Word document, another editor — is stripped of formatting and inserted as plain text. Overrides the existing Markdown-paste auto-conversion while active.

#### RichTextEditor — Markdown Import/Export

- `showMarkdownButton` (default `false`): adds a toolbar button that opens a dialog pre-filled with the current content converted to Markdown. Copy it out to export, or edit it and click Apply to replace the editor content.
- `onMarkdownChange?: (markdown: string) => void`: new callback, fired alongside `onChange` on every content change, with the content as Markdown — independent of the dialog.
...
`v3.7.0` published successfully, but the registry's `readme` metadata field came through empty — the npmjs.com package page showed no README at all, even though the tarball itself contained `README.md`/`README.de.md` correctly (verified via `npm publish --dry-run`). This appears to have been caused by retrying `npm publish` after an OTP (2FA) failure, which can desync the registry's separate readme-metadata write from the tarball upload.

**No code or content changed** — this is a republish to populate the registry metadata correctly. If you're on `3.7.0`, upgrading to `3.7.1` is a no-op functionally; do it only to see the README on npm.
